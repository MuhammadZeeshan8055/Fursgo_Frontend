(function () {
    ...
    function updateSummary() {
        ...
        const total = Math.max(0, selectedService.price + extrasTotal - promoDiscount);
        ...
        if (els.summaryExtras) {
            ...
            if (label) label.textContent = 'Extras & Add-Ons (' + extrasCount + ')';
        }
        ...
        if (promoLabel && appliedPromo) promoLabel.textContent = 'Promo (' + appliedPromo + ')';
    ...
    function getSelectedPetInfo() {
        ...
        const card = document.querySelector('.cbg-pet-card[data-pet-id="' + selectedPetId + '"]');
        if (!card) return null;
        return {
    ...
    function validateNewPetForm(showErrors) {
        ...
        const birthday = document.querySelector('#cbgPetAddNew input[name="birthday"]');
        const breed = document.getElementById('cbgPetBreed');
        const sex = document.querySelector('#cbgPetAddNew input[name="sex"]:checked');
    ...
    function isServiceStepValid() {
        ...
        const outOfArea = value.toLowerCase().includes('manchester') || value.toLowerCase().includes('outside');
    ...
    function updateContinueState() {
        setContinueState(els.continueWraps.addNew, currentStep === 0 && validateNewPetForm(false));
        setContinueState(els.continueWraps.selectExisting, currentStep === 0 && selectedPetId != null);
        setContinueState(els.continueWraps.service, currentStep === 1 && isServiceStepValid());
    ...
    function saveNewPetData() {
        ...
        const birthday = document.querySelector('#cbgPetAddNew input[name="birthday"]').value;
        const type = getSelectedPetType();
        const breed = document.getElementById('cbgPetBreed').value;
        const sex = document.querySelector('#cbgPetAddNew input[name="sex"]:checked')?.value || '';
    ...
    function populateReview() {
        ...
        document.getElementById('cbgReviewPetBreed').textContent = pet?.breed || '—';
        document.getElementById('cbgReviewServiceName').textContent = selectedService.name;
        ...
        if (extrasList) {
            ...
            if (addons.length) {
                addons.forEach(a => {
                    ...
                    row.innerHTML = '<span>' + escapeHtml(a.name) + '</span><span>£' + a.price.toFixed(2) + '</span>';
    ...
    function updateReviewPricing() {
        ...
        if (serviceEl) serviceEl.textContent = '£' + selectedService.price.toFixed(2);
        if (extrasEl) extrasEl.textContent = '£' + extrasTotal.toFixed(2);
        if (extrasLine) extrasLine.style.display = extrasTotal > 0 ? 'flex' : 'none';
    ...
    function updatePromoUI() {
        ...
        document.getElementById('cbgReviewPromoEntry')?.classList.toggle('is-hidden', hasPromo);
    ...
    function getPayableTotal() {
        return Math.max(0, selectedService.price + getAddonsTotal() - promoDiscount);
    }
    ...
    function isCardFormValid() {
        const ids = ['cbgPayFirstName', 'cbgPayLastName', 'cbgPayCardNumber', 'cbgPayExpiry', 'cbgPayCvv', 'cbgPayCity', 'cbgPayPostcode'];
        return ids.every(id => {
            const el = document.getElementById(id);
    ...
    function setupPayStep() {
        ...
        document.getElementById('cbgPayBackBtn')?.addEventListener('click', () => showStep(3));
        ...
        ['cbgPayFirstName', 'cbgPayLastName', 'cbgPayCardNumber', 'cbgPayExpiry', 'cbgPayCvv', 'cbgPayCity', 'cbgPayPostcode'].forEach(id => {
    ...
    function setupPetTypeToggle() {
        document.querySelectorAll('#cbgPetAddNew .pet-option').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#cbgPetAddNew .pet-option').forEach(b => b.classList.remove('highlight'));
    ...
    function setupNewPetValidation() {
        ...
        fields.forEach(id => {
            ...
            if (el) el.addEventListener('input', () => {
                if (id === 'cbgPetName') {
                    document.getElementById('cbgPetNameWrap')?.classList.toggle('valid', el.value.trim().length > 0);
                }
                updateContinueState();
            });
        });

        document.querySelectorAll('#cbgPetAddNew input[name="sex"]').forEach(r => {
            r.addEventListener('change', updateContinueState);
        });

        document.querySelector('#cbgPetAddNew input[name="birthday"]')?.addEventListener('change', updateContinueState);
        document.getElementById('cbgPetBreed')?.addEventListener('change', updateContinueState);
    }
    ...
    function setupPetCards() {
        document.querySelectorAll('.cbg-pet-card').forEach(card => {
            card.addEventListener('click', e => {
                ...
                document.querySelectorAll('.cbg-pet-card').forEach(c => c.classList.remove('selected'));
    ...
    function showAddressPanel(panelId) {
        document.querySelectorAll('.cbg-address-panel').forEach(p => p.classList.remove('active'));
        document.getElementById(panelId)?.classList.add('active');
    }

    function toggleAddressEditUI() {
        const editing = addressEditing;
        document.getElementById('cbgServiceFooter')?.classList.toggle('is-hidden', editing);
        document.getElementById('cbgAddressNormalView')?.classList.toggle('is-hidden', editing);
    }
    ...
    function updateAddressFieldValidation() {
        ...
        document.getElementById('cbgAddrLine1Wrap')?.classList.toggle('valid', !!line1?.value.trim());
        document.getElementById('cbgAddrLine2Wrap')?.classList.toggle('valid', true);
        document.getElementById('cbgAddrCityWrap')?.classList.toggle('valid', !!city?.value.trim());
        document.getElementById('cbgAddrPostcodeWrap')?.classList.toggle('valid', !!postcode?.value.trim());
    }
    ...
    function openAddressEdit() {
        addressEditing = true;
        const homeText = document.getElementById('cbgHomeAddressText')?.textContent || '';
    ...
    function cancelAddressEdit() {
        addressEditing = false;
        showAddressPanel(addressMode === 'home' ? 'cbgAddressHomePanel' : 'cbgAddressDifferentPanel');
        toggleAddressEditUI();
        updateContinueState();
    }
    ...
    function setupServiceStep() {
        ...
        document.querySelectorAll('.cbg-address-toggle-btn').forEach(btn => {
            btn.addEventListener('click', () => setAddressMode(btn.dataset.addressMode));
        });

        document.getElementById('cbgEditHomeBtn')?.addEventListener('click', e => {
            e.stopPropagation();
            openAddressEdit();
        });

        document.getElementById('cbgAddressSaveBtn')?.addEventListener('click', saveAddressEdit);
        document.getElementById('cbgAddressCancelBtn')?.addEventListener('click', cancelAddressEdit);
        document.getElementById('cbgServiceBackBtn')?.addEventListener('click', () => showStep(0));

        document.getElementById('cbgDifferentAddressInput')?.addEventListener('input', updateContinueState);

        ['cbgAddrLine1', 'cbgAddrLine2', 'cbgAddrCity', 'cbgAddrPostcode'].forEach(id => {
            document.getElementById(id)?.addEventListener('input', updateAddressFieldValidation);
        });
    }

    function setupExtrasStep() {
        document.getElementById('cbgExtrasBackBtn')?.addEventListener('click', () => showStep(1));
    }
    ...
    function setupReviewStep() {
        ...
        document.getElementById('cbgReviewBackBtn')?.addEventListener('click', () => showStep(2));

        document.getElementById('cbgConfirmPayBtn')?.addEventListener('click', () => {
            const btn = document.getElementById('cbgConfirmPayBtn');
            if (btn && !btn.disabled) showStep(4);
        });

        document.getElementById('cbgPromoApplyBtn')?.addEventListener('click', applyPromoCode);
        document.getElementById('cbgPromoInput')?.addEventListener('keydown', e => {
            if (e.key === 'Enter') {
                e.preventDefault();
                applyPromoCode();
            }
        });
        document.getElementById('cbgPromoInput')?.addEventListener('input', () => setPromoError(false));
        document.getElementById('cbgRemovePromoBtn')?.addEventListener('click', removePromoCode);
        document.getElementById('cbgReviewRemovePromoBtn')?.addEventListener('click', removePromoCode);

        document.getElementById('cbgReviewTerms')?.addEventListener('change', updateConfirmPayState);
    ...
    function init() {
        ...
        document.getElementById('furs-addons-checkout')?.addEventListener('furs:addons:change', updateSummary);
