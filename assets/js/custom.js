...
(function () {
    ...
    datetimeWrappers.forEach((wrapper) => {
        ...
        function formatDateForInput(d) { return `${pad(d.getDate())} ${monthName(d.getMonth())} ${d.getFullYear()}`; }
...
function go(i, smooth = true) {
    ...
    const viewportW = mobile && window.visualViewport ? window.visualViewport.width : viewport.clientWidth;
...
const onDragEnd = e => {
    ...
    if (delta < -50) go(index + 1); // swipe left
    else if (delta > 50) go(index - 1); // swipe right
